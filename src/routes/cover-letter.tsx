...
export const Route = createFileRoute("/cover-letter")({
  head: () => ({
    meta: [
      { title: "Cover Letter — Resumé.ai" },
      { name: "description", content: "Generate a tailored cover letter from your resume and a job description." },
    ],
  }),
  ...
  const generate = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("cover-letter", {
        body: { resume: r.data, jobDescription: jd, tone, company, role },
      });
      if (error) throw error;
      if ((data as any)?.error) throw new Error((data as any).error);
      setText((data as any).fullText);
    } catch (e: any) {
